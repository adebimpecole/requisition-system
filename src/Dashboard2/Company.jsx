...
const Company = () => {
  ...
  const { user, errorMessage, id } = useContext(Context);
      ...
      .then((querySnapshot) => {
        querySnapshot.forEach((docSnap) => {
          ...
          if (e.original.role == "approver") {
            ...
            const q2 = query(collection2Ref, where("userId", "==", id));

            getDocs(q2)
              .then((querySnapshot) => {
                querySnapshot.forEach((docSnap) => {
                  const companyRef = doc(db, "companies", docSnap.id);
                  const addEmail = e.original.email;

                  updateDoc(companyRef, {
                    approvers: arrayRemove(addEmail),
                  });
    ...
    if (e.original.role == "approver") {
      const swal = Swal.fire({
        html:
          ...
          "Remove Approver" +
          "</span>" +
          "<span class='lil_heading'>Are you absolutely sure you want to remove " +
          e.values.name +
          " as an approver?</span>" +
          "</div>" +
      ...
      const swal = Swal.fire({
        html:
          ...
          "Add Approver" +
          "</span>" +
          "<span class='lil_heading'>Are you absolutely sure you want to add " +
          e.values.name +
          " as an approver?</span>" +
          "</div>" +
          "<div class='confirm_buttons'>" +
      ...
      .then((querySnapshot) => {
        if (!querySnapshot.empty) {
          ...
          approversUserData.forEach((element, index) => {
            approversUserData[
              index
            ].name = `${approversUserData[index].first_name} ${approversUserData[index].last_name}`;
          });
